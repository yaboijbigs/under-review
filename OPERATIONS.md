...
### Prepared public rollout — not executed

[deploy/compose.public.yaml](deploy/compose.public.yaml) prepares **https://underreview.jbigs.com**. This rollout is **prepared, not executed**, pending approval for the dedicated Cloudflare Tunnel. The new images are published and anonymously verified; [deploy/images.json](deploy/images.json) records release candidates, not evidence that the VPS runs them. [RELEASE.md](RELEASE.md) retains the separately identified historical staging image digests and executed checks. No VPS or DNS mutation is claimed for this prepared rollout.
...
The prepared rollout sequence is:
...
3. After backup succeeds, bootstrap applies application setup and syncs the current season, then queues **a new clean-source priority analysis of `2026_02_GB_NYJ`** with a release-specific job key. It uses `backfill: true`, `preferRaw: false`, and `prepareDraft: true`; its early queue timestamp puts this bounded seed ahead of the existing backlog. Start the new web and single worker only after bootstrap succeeds.
4. The one-shot `check` waits up to 30 minutes for the new seed's current revision to contain game-audit v1, a complete winning GB aggregate profile, a checksummed historical reference, a supported historical-outlier flag with at least 20 matches, at least one supported metric, and a dry-run draft tied to that revision. It then requires the game page and ordinary route/readiness checks to succeed. This is a known-case release fixture, not an independent validation of the audit rules. Do not expose a stale report or bypass a failed gate.
5. The tunnel starts **only after** the web is healthy and `check` exits successfully. Then verify public HTTPS, the exact hostname, public report access, protected administration, readiness, and the unchanged unrelated-project baseline. Record actual deployment and route results separately from this preparation.

The prepared manifest retains worker **0.25 CPU / 1536 MiB**, web **0.10 CPU / 512 MiB**, and database **0.15 CPU / 512 MiB** caps. It also limits backup/check to **0.10 CPU / 192 MiB** each, bootstrap to **0.25 CPU / 512 MiB**, and the tunnel to **0.05 CPU / 128 MiB**. One worker, one R/BLAS thread, bounded logs/PIDs, a 15-minute analytics deadline, and a 5 GiB free-disk reserve remain configured. `PRIVATE_STAGING=false` enables public report access, while `VPS_STAGING=true` continues bulk-work restrictions. **Live posting stays disabled, draft-only mode remains selected, and the kill switch stays enabled.** Public website access does not authorize social posting.
...
The public source repository is [yaboijbigs/under-review](https://github.com/yaboijbigs/under-review). The initial public push followed a scan of 141 release files for secrets; subsequent releases are scanned again before pushing. Public web and worker images are built in GitHub Actions and verified through anonymous registry manifest requests. [deploy/images.json](deploy/images.json) now records the **prepared** web source `4bbc02dfb186e633f62f0e8cd83d34d18158d487` and worker source `4d4da0ae0ef19bde46bec12c0298ae6bc3f3bff6`, with their verified immutable digests. These differ from the older images in the executed staging record; publication alone does not establish deployment.
...
The historical executed staging evidence and the separately prepared public release are recorded in [RELEASE.md](RELEASE.md). Local and VPS checks are identified separately. Live X posting remains intentionally untested and disabled; the verified local backup/restore procedure and prepared same-host VPS backup do not replace an encrypted off-host backup schedule.
