...
Public report reads use one database checkout and one consistent SQL snapshot for the selected report, source metadata, revision history and drafts. Automatic prefetching is disabled for report links on both the archive and operator desk. A rendered “Data connection unavailable” notice is a failed request, not an empty or ordinary-game finding. Inspect `web.report.failed`; the pg-pool message `timeout exceeded when trying to connect` means waiting for a pool checkout, whereas an initial connection timeout has a different message. Browser checks must wait for streamed page content while still rejecting actual error views; do not pass them by silently retrying failed reports.

The updated web connection pool permits at most five clients, retains one established connection, and retires surplus idle clients after five minutes. New connection/checkout attempts have a bounded 15-second deadline. Under concurrent SSR on the capped VPS, the former five-second deadline also terminated initial handshakes (`Connection terminated due to connection timeout`); this differs from query execution failure. Keeping established clients available reduces handshake churn. This policy adds no query retry or additional VPS CPU/memory allocation. The separately pinned worker remains on its previously verified image; publishing source changes alone does not update that runtime.
...
Hostinger limits its configuration field to 8,192 characters. The current manifest compacts to **5,381 characters**; deployment programs now live in versioned worker scripts. The helper also supports the preceding manifest's inline JavaScript commands for reproducible rollback, validates their syntax when present, and compares parsed Compose settings without interpolating secrets. It needs installed Node dependencies and Docker Compose, not a running Docker daemon or deployment credentials. Submit the generated file with the existing private environment values; retain the readable source manifest in Git and regenerate after changes.

For application-only container updates, the container workflow accepts `reuse_worker_runtime=true`. It reads the verified immutable worker image from `deploy/images.json`, fetches that image's recorded source commit, and requires `analytics/` and `Dockerfile.worker` to match exactly before reusing its runtime. Analytics tests still run, application trees are replaced completely, and Node dependencies are freshly installed. Changed analytics or runtime recipes require the normal full build; deployment still uses a newly built, tested image digest.

The current manifest's project-scoped upgrade sequence is below. The one-time catch-up import and its coverage verification are complete:
...
3. After backup succeeds, bootstrap applies additive migrations and ensures the existing operator account. `AUDIT_CATCHUP_WEEKS=1,2` queues versioned audit upgrades for the explicitly authorized weeks. It does not reimport old bundles or directly run R. Web and worker can start after bootstrap. Most upgrades preserve existing R results; a legacy raw report with an earlier clean report requires fresh clean reconciliation under the same worker job before saving. Unrelated refresh errors retain normal failure/retry behavior.
4. Worker startup checks for reports that regressed from clean to raw data. It appends a transparent restoration only when the saved clean source is complete and its meaningful findings equal the current report. If scores, metrics, profiles, audit, or events differ, it queues fresh clean analysis instead. Historical reports remain available and stale human approvals stay stale. Dispatch rechecks source maturity, and persistence rejects raw-over-clean writes. Normal automatic season discovery then resumes.
5. Once web and worker are healthy, `scripts/public-check.ts` waits up to 30 minutes for the seed report's schema-validated game audit, supported metric, validated GB **Sus** rating, and successful web routes. Existing reports stay readable while audit upgrades run. Website readiness does not require a social draft or a human ruling assessment; the separate social-posting guard is still checked. `scripts/public-coverage.ts` waits up to one hour for all 32 Week 1–2 schedule records, all 32 scored games, and the current audit version on every scored game's report. It polls once per minute; this proves audit presence, not that every optional field/model is supported. Inspect missing-data states separately.
...
Missing paired team totals and missing referee assignments are checked in **15-minute buckets** for scored games in the configured season. These lightweight jobs run alongside the single R-analysis lane, so maintenance analysis of another game does not hold up checks or X delivery. Pending/running checks coalesce, and per-game locks still prevent concurrent report changes. The provider can publish later than game end; the interval is a retry cadence, not a promised posting time.

Aggregate completion validates the new totals against saved final play-by-play and reuses the existing model output. Referee completion updates the existing expectations only after an attributable schedule supplies a name; insufficient historical referee samples are not mistaken for a missing assignment. Changed non-referee schedule facts require normal clean analysis. Unchanged or still-missing data creates no report revision. New evidence preserves earlier revisions and the existing one-initial-post-per-game/account guard; later referee updates never automatically repost an already-published game. Full clean-data reconciliation retains its separate 6/24/48-hour checks.
...
The official requirements and URLs are listed in [DATA_SOURCES.md](DATA_SOURCES.md). The operator needs an X developer application with appropriate paid API access; setup does not purchase anything. Configure a confidential Web App / Automated App & Bot with read and write access, an exact callback URI, [OAuth 2.0 authorization code with PKCE S256](https://docs.x.com/fundamentals/authentication/oauth-2-0/authorization-code), and `tweet.read tweet.write users.read offline.access` scopes. Set `X_CLIENT_ID`, `X_CLIENT_SECRET`, `X_REDIRECT_URI`, and a persistent 32-byte hex `TOKEN_ENCRYPTION_KEY` on the server. Encrypted tokens cannot be recovered if that key is lost. The app uses the [official create-post API](https://docs.x.com/x-api/posts/create-post), with original informational posts and explicit operator activation under [X's automation rules](https://help.x.com/en/rules-and-policies/x-automation).

For the public site, register `https://underreview.jbigs.com` as the website and `https://underreview.jbigs.com/api/x/callback` as the exact callback; set that same callback in `X_REDIRECT_URI`. Set `X_EXPECTED_USERNAME=wasitrigged` to bind OAuth connection, activation and delivery to the intended author. This setting is separate from the footer's `SOCIAL_HANDLE`. The administration readiness panel checks presence and account metadata without displaying credentials or submitting a post. API credits/billing must be checked in the X developer console: the [official pricing page](https://docs.x.com/x-api/getting-started/pricing) currently lists posts containing a URL at **$0.200 per request** (verified 2026-09-23); 272 regular-season game posts would cost approximately $54.40 before reads or approved corrections. Setup does not purchase credits.

Connect the intended account through authenticated administration. Keep draft-only mode and the kill switch enabled until an operator deliberately enables live deployment settings, selects that account, activates automatic mode, and disables the kill switch. Releasing the kill switch records a fresh cutoff: **only games kicking off after that activation are eligible**, then only after final data validates. Games already underway, historical backfills, and games during a disabled interval remain preview-only. One original post per game/account includes the final score, the report's editorial game rating, its main supported reason, and a revision-pinned report URL. Automatically generated posts use structured report evidence, never visitor comments. The live outbox row and delivery job commit together, and the game/account unique guard also blocks duplicates from later revisions or unknown delivery outcomes. Material corrections/updates remain manual and explicitly labeled; create a correction/update draft from the later revision, inspect it, then approve it. Verify the exact report URL is reachable before publishing.
...
The `game-final-screening-v2` templates use the five emoji rating headings, an away/home score line, a supported finding, `See the Review:`, and `#NFL #UnderReview`. Debatable reports with review candidates show their count; a spread-only Debatable report explains the spread instead. Sus describes a strong fairness concern, and RIGGED? requires both qualifying evidence types. Text is checked using X's weighted 280-character limit; shorter complete finding variants preserve the evidence, full revision link, and hashtags. Manual corrections and updates retain their explicit prefixes. Ready publication jobs run before maintenance reanalyses; active analysis jobs finish normally and per-game locks remain in force.
