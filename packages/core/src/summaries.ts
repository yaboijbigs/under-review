...
export function reportSummary(game:Game,analysis:AnalysisResult):string{
 ...
 if(analysis.gameAudit?.version==='under-review-game-audit-v5'||analysis.gameAudit?.version==='under-review-game-audit-v6'){const verdict=getGameVerdict(analysis.gameAudit);return `${score} ${verdict.label}${verdict.rating?` ${verdict.rating}/5`:''}. ${verdict.summary}`;}
