...
export function GameCard({game}: {game: CardData}) {
  ...
  return <article className={`game-card verdict-${verdict.tone}`} data-verdict={verdict.level}>
    ...
    <div className="card-bottom"><span>{verdict.reviewCount ? `${verdict.reviewCount} key ${verdict.reviewCount === 1 ? "play" : "plays"} to inspect` : game.revisionNumber !== null ? "Automatic analysis complete" : "Runs after final data arrives"}</span><Link prefetch={false} href={href}>See why <span aria-hidden="true">↗</span></Link></div>
  </article>;
}
...
export function WpTimeline({points: raw, home, away}: {points: AnalysisResult["timeline"]; home: string; away: string}) {
  const supported = (value: number | null): value is number => value !== null && Number.isFinite(value) && value >= 0 && value <= 1;
  const points = raw.map((play,index)=>({index,probability:play.homeWp})).filter((point): point is {index:number;probability:number}=>supported(point.probability));
  ...
  const isolated = points.length===1 ? points : [];
  const overtime:{start:number;end:number}[]=[];
  raw.forEach((play,index)=>{
    if(play.quarter === null || play.quarter <= 4)return;
  ...
  const overtimeMissing=raw.filter(play=>play.quarter!==null && play.quarter>4 && !supported(play.homeWp)).length;
  return <div className="wp-chart">
    <div className="chart-legend"><span><i />{teamName(home)} chance to win, before each play</span><span className="muted">Regulation model · vs. {teamName(away)}</span></div>
    <div className="chart-coverage"><p><strong>{points.length} supported estimates</strong> across {raw.length} recorded game entries.</p>{missing>0 && <p><strong>{missing} entries have no supported estimate</strong>{overtimeMissing>0 ? <>: {overtimeMissing} overtime entries{missing>overtimeMissing ? ` and ${missing-overtimeMissing} other recorded states` : ""}</> : null}.</p>}{overtime.length>0 && <p className="chart-overtime-note"><span className="overtime-swatch" aria-hidden="true"/>{points.length ? "Overtime is shaded." : "This game includes overtime."} This model covers regulation only.</p>}</div>
    {points.length ? <svg viewBox="0 0 960 245" role="img" aria-labelledby="wp-title wp-desc">
      <title id="wp-title">{`${teamName(home)} preplay chance to win during the game`}</title><desc id="wp-desc">{points.length} supported preplay estimates across {raw.length} recorded game entries. {missing} entries have no supported estimate. The line connects available estimates; intermediate values are not calculated.{overtime.length>0 ? " The shaded region marks overtime, which is not modeled." : ""} The line stops at the last available estimate. All recorded entries, including those without estimates, appear in the table below.</desc>
      {overtime.map(range=>{const left=Math.max(44,x(range.start-.5)),right=Math.min(936,x(range.end+.5)),width=right-left;return <g className="chart-overtime-region" key={range.start}><rect x={left} y="24" width={width} height="180" fill="#dce2d3"/><line x1={left} x2={left} y1="24" y2="204" stroke="#788171" strokeDasharray="4 4"/>{width>=90 && <><text x={left+width/2} y="101" textAnchor="middle" fontSize="13" fill="#4d5c44">OVERTIME</text><text x={left+width/2} y="122" textAnchor="middle" fontSize="11" fill="#4d5c44">Not modeled</text></>}</g>;})}
    ...
    </svg> : <p className="chart-no-estimates">No supported estimates are available for this chart. You can still inspect every recorded entry below.</p>}
    <p className="chart-interpolation-note">Line connects available estimates; intermediate values are not calculated.</p>
    <p className="chart-preplay-note">The horizontal axis follows recorded entries, not elapsed game time. Estimates are made before a play, so the line stops at the last available estimate rather than being forced to 0% or 100%.</p>
    <details className="chart-table"><summary>View all chart data ({raw.length} entries, including gaps)</summary><div className="table-scroll"><table><caption className="sr-only">{teamName(home)} preplay chance to win for every recorded game entry</caption><thead><tr><th scope="col">Period</th><th scope="col">Clock</th><th scope="col">Source play</th><th scope="col">{home} preplay chance</th></tr></thead><tbody>{raw.map((play,index)=><tr className={supported(play.homeWp) ? "" : "chart-estimate-unavailable"} key={`${play.playId}:${index}`}><td>{play.quarter===null ? "—" : play.quarter>4 ? play.quarter===5 ? "OT" : `OT ${play.quarter-4}` : `Q${play.quarter}`}</td><td>{play.clock || "—"}</td><td><a href={`#play-${encodeURIComponent(play.playId)}`}>Play {play.playId} ↗</a></td><td>{supported(play.homeWp) ? `${(play.homeWp*100).toFixed(1)}%` : play.quarter!==null && play.quarter>4 ? "Not modeled (overtime)" : "No supported estimate"}</td></tr>)}</tbody></table></div></details>
  </div>;
}
