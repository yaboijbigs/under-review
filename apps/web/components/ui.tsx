...
export function GameCard({game}: {game: CardData}) {
  ...
  return <article className={`game-card verdict-${verdict.tone}`} data-verdict={verdict.level} data-rating={verdict.rating ?? "unrated"}>
    ...
    <p className="card-rating-label">GAME SUSPICION RATING <span>{verdict.rating ? `${verdict.rating} / 5` : "—"}</span></p>
    <div className="card-verdict"><span className="verdict-indicator" aria-hidden="true"/><h3><Link prefetch={false} href={href}>{verdict.label}</Link></h3></div><p className="card-rating-definition">{verdict.definition}</p>
    ...
    <div className="card-finding"><p>{verdict.rating && verdict.rating <= 2 ? verdict.summary : verdict.reasons[0] || verdict.summary}</p><span className="card-rating-boundary">Automatic screening · Not a finding of manipulation</span></div>
