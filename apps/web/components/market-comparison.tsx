...
export function MarketComparison({market,currentRules=false}:{market:MarketAudit|undefined;currentRules?:boolean}){
  ...
  return <section className={`market-comparison market-${market.surprise}`} aria-labelledby="market-title">
    ...
    <details className="market-method"><summary>Where this comparison comes from</summary><p>The recorded closing spread is an expected winning margin, not the final score. We compare the size of the miss with earlier seasons: {referencePeriod(ref.startSeason,ref.endSeason)}. At least 500 prior games are required to affect the rating.</p><p>{currentRules?'The rating uses an add-one tail estimate and a three-comparison adjustment. Adjusted frequencies at most 20% qualify for Debatable; at most 10% qualify for Hmm.':'A result in the most unusual 10% can raise a game to Debatable; the most unusual 5% can raise it to Hmm.'} Spread alone cannot produce Sus or RIGGED?.</p>{market.source&&<a href={market.source.url} target="_blank" rel="noreferrer">View the source data ↗</a>}</details>
  </section>;
}
