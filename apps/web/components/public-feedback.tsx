...
import type { PublicVisitorFeedback,PublicFeedbackPage } from '@under-review/core/visitor-feedback';
...
export function PublicFeedback({gameId}:{gameId:string}){
 ...
 useEffect(()=>{
  ...
  read(null,controller.signal).then(data=>{setEntries(data.entries);setNextCursor(data.nextCursor);setLoaded(true);}).catch(()=>{if(!controller.signal.aborted)setError('Fan feedback is temporarily unavailable. Please try again.');}).finally(()=>{if(!controller.signal.aborted)setLoading(false);});
 ...
 async function more(){
  ...
  try{const data=await read(nextCursor);setEntries(current=>{const known=new Set(current.map(entry=>entry.id));return [...current,...data.entries.filter(entry=>!known.has(entry.id))];});setNextCursor(data.nextCursor);}
 ...
 return <section id="fan-feedback" className="report-section public-feedback" aria-labelledby="fan-feedback-title">
  ...
  {entries.length>0&&<div className="visitor-feedback-list">{entries.map(entry=><article key={entry.id}>
   <div className="visitor-feedback-top"><strong>{SUSPICION_SCALE[entry.rating-1].label} · {entry.rating}/5</strong><span>{entry.agreement==='agree'?'Agrees':'Disagrees'} with {SUSPICION_SCALE[entry.modelRating-1].label}</span></div>
   {entry.comment?<p className="visitor-comment">{entry.comment}</p>:<p className="small muted">Rating only · no explanation added.</p>}
  ...
  {loaded&&!loading&&!error&&entries.length===0&&<p className="feedback-empty">No public feedback yet. Be the first to share your take.</p>}
