...
export function GameVerdict({audit}: {audit: GameAudit | undefined}) {
  ...
  return <section className={`game-verdict verdict-${verdict.tone}`} aria-labelledby="verdict-title" data-verdict={verdict.level} data-rating={verdict.rating ?? "unrated"}>
    <div className="verdict-main"><p className="eyebrow"><span className="verdict-indicator" aria-hidden="true"/>GAME RATING</p><div className="rating-headline"><h2 id="verdict-title">{verdict.label}</h2>{verdict.rating && <span className="rating-number" aria-label={`Level ${verdict.rating} of 5`}>{verdict.rating}<small> / 5</small></span>}</div>
