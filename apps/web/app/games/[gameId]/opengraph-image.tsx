...
export default async function ReportImage({params}: {params: Promise<{gameId:string}>}) {
  ...
  return new ImageResponse(<div style={{display:"flex",flexDirection:"column",width:"100%",height:"100%",padding:"45px 65px",background:"#20231f",color:"#f4f3ed",fontFamily:"sans-serif",borderTop:`14px solid ${color}`}}>
    ...
    <div style={{display:"flex",fontSize:15,color:"#acb89d",marginTop:24}}>GAME RATING</div><div style={{display:"flex",fontSize:51,lineHeight:1.12,fontWeight:800,color,marginTop:8}}>{verdict.label}{verdict.rating ? ` · ${verdict.rating}/5` : ""}</div>
    <div style={{display:"flex",fontSize:20,lineHeight:1.4,marginTop:16,color:"#d2d9c9",maxHeight:100,overflow:"hidden"}}>{verdict.reasons.length ? verdict.reasons.slice(0,2).join(' ') : verdict.summary}</div>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginTop:"auto",paddingTop:20,borderTop:"1px solid #48523e"}}><span style={{fontSize:17,color}}>{report ? 'SEE THE FULL GAME REPORT' : 'WAITING FOR FINAL GAME DATA'}</span><span style={{fontSize:15,color:"#acb89d"}}>{verdict.reviewCount ? `${verdict.reviewCount} key plays to inspect` : 'See the evidence. Understand the result.'}</span></div>
  </div>,size);
}
