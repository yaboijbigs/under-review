...
export default async function Home({searchParams}: {searchParams: Promise<Record<string, string | string[] | undefined>>}) {
  ...
  return <>
    <section className="home-intro"><div><p className="eyebrow"><span className="accent-line"/>NFL / {season} SEASON</p><h1>Was that game<br/><span>unusual?</span></h1><p>The score. The penalties. The referee’s history.<br/>See what stood out—and have your say.</p></div><div className="intro-aside"><span className="issue-number">AUTOMATIC POSTGAME REPORTS</span><p>Go beyond the score.<br/><strong>Get a clear verdict.</strong></p><span>Reports appear after final game data arrives and update as the data improves.</span></div></section>
    <section className="archive-section" aria-labelledby="archive-title">
      <div className="section-heading"><div><p className="eyebrow">THE GAMES, EXPLAINED</p><h2 id="archive-title">Game reports <span className="count">{games.length}</span></h2></div><span className="section-caption">Five levels of scrutiny. The evidence explains each rating.</span></div>
