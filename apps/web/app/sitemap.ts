...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  try { const games = await listGames({publishedOnly:true}); return [...pages, ...games.map(game => ({url:new URL(`/games/${encodeURIComponent(game.id)}`,config.siteUrl).href, lastModified:game.updatedAt || undefined, changeFrequency:"weekly" as const}))]; } catch {return pages;}
}
