...
const {positionals,values}=parseArgs({allowPositionals:true,options:{season:{type:'string'},game:{type:'string'},backfill:{type:'boolean'},clean:{type:'boolean'},username:{type:'string'},role:{type:'string'},input:{type:'string'},output:{type:'string'},confirm:{type:'boolean'}}});
...
try{
 ...
 switch(command){
  ...
  default:result={commands:['migrate','doctor','sync-season --season 2026','analyze-game --game 2026_01_NE_SEA [--backfill] [--clean]','backfill --season 2023','reconcile --season 2026','draft --game 2026_01_NE_SEA','admin:create','train|calibrate|evaluate|percentiles --input manifest.json --output result.json']};
 }
