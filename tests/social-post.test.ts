...
import { renderSocialPost,validateSocialPost } from '../packages/core/src/social-post.js';
...
describe('original automatic X final report prose',()=>{
 it.each([[1,'Sus (4/5)'],[2,'Hmm (3/5)'],[3,'Fair (1/5)']] as const)('preserves the supported editorial scale for %i wins', (wins,label)=>{
  ...
  expect(draft.text).toContain('GB 23–20 NYJ | Final. Game rating: '+label);expect(draft.text.endsWith(reportUrl)).toBe(true);
 ...
 it('uses the exact top scale label as an editorial question with both kinds of evidence',()=>{
  ...
  expect(draft.text).toContain('Game rating: RIGGED? (5/5)');expect(draft.text).toContain('1 win in 20');expect(draft.text).toContain('3 drive-extending penalties');
 ...
 it('keeps unsupported and sparse profiles unrated',()=>{
  for(const a of [undefined,audit(0,14)]){const draft=renderSocialPost(game,analysis(a),reportUrl);expect(draft.text).toContain('Game rating: Unrated.');expect(draft.text).toContain('too limited');expect(draft.evidenceIds).toEqual([]);expect(draft.valid).toBe(true);}
 });
 it.each([2,3] as const)('explains a market-only level %i with the recorded line, actual result and attributable error',floor=>{
  ...
  expect(draft.text).toContain(`Game rating: ${floor===3?'Hmm':'Debatable'} (${floor}/5)`);expect(draft.text).toContain('Recorded line: NYJ -3; GB won by 3. 6 points off');expect(draft.text).toContain(`${floor===3?25:50}/500 prior games`);
 ...
 it('does not publish unvalidated market arithmetic or inflate the resulting rating',()=>{
  const a=audit(3);a.market=market(3);a.market.absoluteError=100;const draft=renderSocialPost(game,analysis(a),reportUrl);expect(draft.text).toContain('Game rating: Fair (1/5)');expect(draft.text).not.toContain('100 points');expect(draft.evidenceIds).toEqual([]);
 });
 it('states a review flag without inventing a call judgment',()=>{
  const a=audit(3);addCluster(a,1);const draft=renderSocialPost(game,analysis(a),reportUrl);expect(draft.text).toContain('Game rating: Debatable (2/5)');expect(draft.text).toContain('1 play needs a closer look');expect(draft.text).not.toContain('Raw visitor');
 });
 it('leads with the stronger cluster when historical evidence only reaches a lower rating',()=>{
  const a=audit(2);addCluster(a,3);const draft=renderSocialPost(game,analysis(a),reportUrl);expect(draft.text).toContain('Game rating: Sus (4/5)');expect(draft.text).toContain('3 defensive penalties extended one GB drive');
 });
 it('makes manually requested correction/update text explicit and pins its revision',()=>{
  for(const kind of ['correction','update'] as const){const draft=renderSocialPost(game,analysis(audit()),reportUrl,kind);expect(draft.text.toLowerCase().startsWith(kind+':')).toBe(true);expect(draft.text.endsWith('?revision=2')).toBe(true);expect(draft.valid).toBe(true);}
 });
