...
describe.skipIf(process.env.RUN_DB_TESTS!=='1')('published game listing (isolated temporary schema)',()=>{
 ...
 it('keeps the latest season ahead of historical high-week games before the 400-row limit',async()=>{
  const base:Game={id:'2100_01_TST_DEM',season:2100,week:1,gameType:'REG',homeTeam:'DEM',awayTeam:'TST',homeScore:17,awayScore:10,kickoffAt:'2100-09-10T17:00:00.000Z',providerData:{synthetic:true}};
  const current=[base,{...base,id:'2100_02_TST_DEM',week:2,kickoffAt:'2100-09-17T17:00:00.000Z'},
   {...base,id:'2100_02_ABC_DEM',awayTeam:'ABC',week:2,kickoffAt:'2100-09-17T20:00:00.000Z'}];
  await repository.saveGames(current);
  await db.query(`INSERT INTO games(id,season,week,game_type,home_team,away_team,kickoff_at,game_json)
   SELECT 'synthetic-old-high-week-'||n,2098,22,'SB','DEM','TST','2099-02-10T23:00:00Z'::timestamptz,
   jsonb_build_object('id','synthetic-old-high-week-'||n,'season',2098,'week',22,'gameType','SB','homeTeam','DEM','awayTeam','TST','homeScore',null,'awayScore',null,'kickoffAt','2099-02-10T23:00:00Z','providerData',jsonb_build_object('synthetic',true))
   FROM generate_series(1,401) n`);
  const expected=[current[2].id,current[1].id,current[0].id];
  const allSeasons=await repository.listGames();
  expect(allSeasons).toHaveLength(400);expect(allSeasons.slice(0,3).map(row=>row.id)).toEqual(expected);
  expect((await repository.listGames({season:2100})).map(row=>row.id)).toEqual(expected);
 });
});
