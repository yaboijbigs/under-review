...
Reports lead with a five-level game suspicion rating: **Fair → Debatable → Hmm → Sus → RIGGED?**, with **Unrated** outside the scale when evidence is insufficient. This versioned editorial screening ladder combines supported historical patterns and repeated drive-extending penalties; it is not a probability or finding of manipulation. Routine review-candidate counts cannot inflate the rating, and the top tier requires both signals to favor the same winner. Reports show the exact counts, reasons and plays behind their rating. Automatic analysis, source maturity, and human review remain separate. Once cleaned play-by-play has been analyzed, raw jobs cannot replace it. Startup repairs an equivalent older source regression transparently, or queues fresh clean analysis when the evidence differs.
